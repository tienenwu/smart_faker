...
- 🚀 **モックサーバースイート**：REST・GraphQL・SSE/WebSocket・録画/リプレイに対応（v0.5.0新機能！）
- 🧰 **スキーマインポートとCLI**：JSON Schema / OpenAPI / Prisma を取り込み、新しいCLIでデータ生成（v0.5.0新機能！）
- 🔐 **プライバシーツールキット**：マスキング、匿名化、合成データ生成をサポート（v0.5.0新機能！）
...
## APIモックとストリーミング - v0.5.0新機能！

SmartFaker 0.5.0 ではモックサーバーが REST / GraphQL / SSE / WebSocket に対応し、録画・リプレイや CLI からの操作も可能になりました。詳しい英語ドキュメントは本家 README を参照してください。以下は代表的なコード例です。
